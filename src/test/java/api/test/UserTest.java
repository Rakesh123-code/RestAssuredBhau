...
import io.restassured.response.Response;
	...
	public class UserTest {
	 ...
	 //       payload.setUsername("test_" + System.currentTimeMillis());
	    ...
	    //	logger=Logger.getLogger(UserTest.class);
	    ...
	    public void TestGetUser() throws InterruptedException {
	        ...
	        Response response = ApiUtils.retryApiCall(() -> UserEndPoints.ReadUser(payload.getUsername()), 5, 9000);
	        response.then().log().all();
	        Assert.assertEquals(response.getStatusCode(), 200);
...
//	        Assert.assertEquals(response.getStatusCode(), 200);
	        ...
	        Response response = ApiUtils.retryApiCall(() -> UserEndPoints.UpdateUser(oldUsername,payload), 5, 9000);
	        Assert.assertEquals(response.getStatusCode(), 200);
...
//	       Assert.assertEquals(response.getStatusCode(), 200, "User deletion filed!");
	       ...
	       Response response1 = ApiUtils.retryApiCall(() -> UserEndPoints.DeleteUser(payload.getUsername()), 5, 9000);
